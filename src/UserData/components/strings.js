import LocalizedStrings from "react-localization";

const strings = new LocalizedStrings({
  en: {
    projectCannotBeEmpty: "Project name cannot be empty",
    addingProjectFailed: "Adding project failed. Please try again later",
    addProject: "Add project",
    projectName: "Project name",
    hourlyRate: "Hourly rate",
    add: "Add",
    close: "Close",
    remove: "Remove",
    removeProjectNote:
      "By removing this project all working data and trip logs corresponding to this project will also be removed. Are you sure you want to remove the project?",
    removeProject: "Remove project",
    timeAlreadyAdded: "Work time already added",
    addingWorktimeFailed: "Adding work time failed. Please try again later.",
    start: "Starting",
    end: "Ending",
    project: "Project",
    projects: "Projects",
    noAddedProjects: "No added projects",
    quickSelect: "Quick selection",
    removingWorktimeFailed: "Removing working time failed. Please try again later.",
    quickSelection: "Quick work time selections",
    accountActive: "Your account is active until",
    until: "",
    accountExpired: "Your account has expired on",
    deleteMyAccount: "Delete account",
    deleteNote:
      "Deleting your user account will remove all your data from the application. Are you sure you want to delete your account?",
    password: "Password",
    checkPassword: "Invalid password",
    accountRemoved: "Your account has been removed",
    thanksForTheFeedback: "Thank you for your feedback!",
    thanksForTheReview: "Thank you for reviewing the application!",
    errorSendingFeedback: "Sending feedback failed. Please try again later.",
    errorSendingReview: "Sending review failed. Please try again later.",
    sendFeedback: "Send feedback",
    chooseTopicForFeedback: "Choose a topic for your feedback",
    feedbackCannotBeEmpty: "Feedback cannot be empty",
    suggestions: "Suggestions",
    complaints: "Complaints",
    other: "Other",
    send: "Send",
    feedback: "Feedback",
    updateReview: "Update your review",
    reviewTheApplication: "Review the application - receive 10% discount on your next purchase!",
    invalidRating: "Invalid rating",
    reviewCannotBeEmpty: "Review cannot be empty",
    alias: "Alias",
    yourRating: "Your rating",
    review: "Review",
    update: "Update",
    subject: "Subject",
    inviteUsers: "Invite users",
    inviteInfo:
      "Send this link to a friend. You will receive a permanent 2.00 € discount on your annual price for each new user that has made a purchase in the application, if the invited user used your link to register to the application. The invited user receives a 20% discount on their first purchase as well.",
    annualPriceInfo:
      "Annual price minimum 29.90 €, when 10 users invited by you have made a purchase in the application.",
    linkCopied: "Link copied to clipboard",
    serverDidNotResponsd: "Server did not responsd. Please try again later.",
  },
  fi: {
    projectCannotBeEmpty: "Projektin nimi on täytettävä",
    addingProjectFailed: "Projektin lisääminen ei onnistunut. Yritä myöhemmin uudelleen.",
    addProject: "Lisää projekti",
    projectName: "Projektin nimi",
    hourlyRate: "Tuntilaskutus",
    add: "Lisää",
    close: "Sulje",
    remove: "Poista",
    removeProjectNote:
      "Poistamalla tämän projektin myös kaikki projektiin liittyvät työajat ja ajotiedot poistetaan. Oletko varma, että haluat poistaa projektin?",
    removeProject: "Poista projekti",
    timeAlreadyAdded: "Aika on jo lisätty",
    addingWorktimeFailed: "Työajan lisääminen ei onnistunut. Yritä myöhemmin uudelleen.",
    start: "Aloitus",
    end: "Lopetus",
    project: "Projekti",
    projects: "Projektit",
    noAddedProjects: "Ei lisättyjä projekteja",
    quickSelect: "Pikavalinta",
    removingWorktimeFailed: "Työajan poistaminen ei onnistunut. Yritä myöhemmin uudelleen.",
    quickSelection: "Työaikojen pikavalinnat",
    accountActive: "Tilisi on aktiivinen",
    until: " asti",
    accountExpired: "Tilisi käyttöoikeus on päättynyt",
    deleteMyAccount: "Poista käyttäjätili",
    deleteNote:
      "Poistamalla tilisi kaikki sovellukseen tallentamasi tiedot poistetaan. Oletko varma, että haluat poistaa tilisi?",
    password: "Salasana",
    checkPassword: "Tarkista salasana",
    accountRemoved: "Käyttäjätilisi on poistettu",
    thanksForTheFeedback: "Kiitos palautteesta!",
    thanksForTheReview: "Kiitos arvostelusta",
    errorSendingFeedback:
      "Palautteen antaminen ei onnistunut. Ole hyvä ja yritä myöhemmin uudelleen.",
    errorSendingReview:
      "Arvostelun antaminen ei onnistunut. Ole hyvä ja yritä myöhemmin uudelleen.",
    sendFeedback: "Anna palautetta",
    chooseTopicForFeedback: "Valitse palautteen aihe",
    feedbackCannotBeEmpty: "Palaute ei voi olla tyhjä",
    suggestions: "Ehdotukset",
    complaints: "Vikailmoitukset",
    other: "Muu",
    send: "Lähetä",
    feedback: "Palaute",
    updateReview: "Päivitä arvostelusi",
    reviewTheApplication: "Arvostele sovellus - saat 10% alennuksen seuraavasta ostoksestasi!",
    invalidRating: "Virheellinen arvosana",
    reviewCannotBeEmpty: "Arvostelu ei voi olla tyhjä",
    alias: "Nimimerkki",
    yourRating: "Mielipiteesi",
    review: "Arvostelu",
    update: "Päivitä",
    subject: "Aihe",
    inviteUsers: "Kutsu käyttäjiä",
    inviteInfo:
      "Lähetä tämä linkki tuttavillesi. Saat pysyvän 2,00 € alennuksen vuosihintaasi jokaista kutsumaasi oston tehnyttä käyttäjää kohden, jos käyttäjät rekisteröityvät antamasi linkin kautta! Kutsuttu käyttäjä saa 20% alennuksen ensimmäisestä ostoksestaan.",
    annualPriceInfo:
      "Vuosihinta alimmillaan 29,90 €, kun 10 kutsumaasi käyttäjää on tehnyt sovelluksessa oston.",
    linkCopied: "Linkki kopioitu",
    serverDidNotResponsd: "Palvelimeen ei saatu yhteyttä. Ole hyvä ja yritä myöhemmin uudelleen.",
  },
});

export default strings;
